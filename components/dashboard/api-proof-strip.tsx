    ...
    >
      <header className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          >
            All {tickers.length} active naira pairs (out of 12 listed total), watch them tick ⚡
          </p>
        </div>
        <div
      ...
      <div
        className="card-elev overflow-x-auto rounded-xl p-5 md:p-6"
        role="region"
        ...
        <ul className="flex min-w-max gap-3">
          {tickers.length === 0 ? (
            <li className="px-3 py-2 text-xs text-muted-foreground">
              No naira markets right now. See the chip above for why.
            </li>
          ) : (
      ...
      <p className="mt-3 flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
        ...
        <span>
          This strip checks <span className="text-foreground/80">/api/markets</span> every 15 seconds. The route validates Quidax's response, caches it for everyone, and falls back to a clearly labelled stale or empty state if Quidax is down. It never invents prices. 🔒
        </span>
      </p>
    </section>
  )
...
function PairPill({
  ...
  return (
    <li
      className={`group flex shrink-0 items-center gap-4 rounded-lg border border-border/60 bg-card/40 px-4 py-3 ring-1 ring-transparent transition-all duration-300 ${flashCls}`}
    >
