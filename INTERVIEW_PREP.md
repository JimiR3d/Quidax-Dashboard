...
> *"Retail volume on Nigerian exchanges has plateaued. The fastest-growing flow is businesses — fintechs, SMEs, remittance operators — using stablecoins to settle dollars in and out of Nigeria. Quidax is the only locally-licensed exchange with deep NGN liquidity, a production-grade API, and the only one listing cNGN, the regulated naira stablecoin. That puts them in a quadrant nobody else can enter quickly. My dashboard quantifies the opportunity in four B2B segments, maps it onto four real corridors, lets the reader plug in their own assumptions, and proposes five concrete moves to capture it."*
...
- **Luno** — Singapore-owned, conservative product surface, trusted brand. *They have no NGN-native B2B API. Not a B2B competitor.*
...
|---|---|---|---|
| **NG → CN** | $22.5B | Largest single trade corridor. 320 bps + 4 days via banks vs 90 bps + 8 minutes via USDT. | USDT/NGN, USDC/NGN |
| **NG → AE (Dubai)** | $6.8B | Re-export, gold, luxury, diaspora savings. Banking corridor under chronic FX pressure. | USDT/NGN |
| **NG ↔ KE** | $1.2B | Regional B2B (SaaS, logistics, agritech). AfCFTA tailwind. Small but high-margin. | USDT/NGN |
| **DIASPORA → NG** | $20.9B | World Bank's #1 SSA recipient. Stablecoins already disrupting MTOs. | USDT/NGN, CNGN/NGN, USDC/NGN |
